...
public class PaymentTransferBackgroundService : BackgroundService
{
    ...
    private async Task ProcessScheduledRefundsAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var payment in scheduledRefunds)
        {
            ...
            {
                await ProcessPartialRefundWithTransferAsync(payment, stripeConnectService, paymentRepository,
                    unitOfWork);
            }
    ...
    private async Task ProcessPartialRefundWithTransferAsync(Payment payment,
        ...
        IUnitOfWork unitOfWork)
    {
        // Only increment attempts for operations we're actually going to try
        bool needsRefund = string.IsNullOrEmpty(payment.StripeRefundId);
        bool needsTransfer = string.IsNullOrEmpty(payment.StripeTransferId);

        if (needsRefund) payment.RefundAttempts += 1;
        if (needsTransfer) payment.TransferAttempts += 1;

        try
        {
            _logger.LogInformation(
                $"Processing partial refund for Payment ID: {payment.Id}, " +
                $"Refund: {payment.RefundAmount} {payment.Currency.ToUpper()} (Needed: {needsRefund}), " +
                $"Transfer: {payment.SellerAmount} {payment.Currency.ToUpper()} (Needed: {needsTransfer}), " +
                $"Attempt: Refund={payment.RefundAttempts}, Transfer={payment.TransferAttempts}");
            ...
            {
                _logger.LogInformation(
                    $"Refund already completed for Payment ID: {payment.Id}, Refund ID: {payment.StripeRefundId}");
            }
            ...
            if (needsTransfer)
            {
                ...
                _logger.LogInformation($"Transfer successful for Payment ID: {payment.Id}, Transfer ID: {transfer.Id}");
            }
            else
            {
                _logger.LogInformation(
                    $"Transfer already completed for Payment ID: {payment.Id}, Transfer ID: {payment.StripeTransferId}");
            }

            // Mark as complete only if both operations are done
            ...
            _logger.LogInformation(
                ...
                $"Refund ID: {payment.StripeRefundId}, Transfer ID: {payment.StripeTransferId}");
        }
        catch (Exception ex)
        {
            bool refundFailed = needsRefund && string.IsNullOrEmpty(payment.StripeRefundId);
            bool transferFailed = needsTransfer && string.IsNullOrEmpty(payment.StripeTransferId);

            if (payment.RefundAttempts >= MaxRetryAttempts || payment.TransferAttempts >= MaxRetryAttempts)
            {
                payment.Status = PaymentStatus.Failed;
                payment.Notes = AddNote(payment.Notes,
                    $"Partial refund process failed after max attempts. Refund completed: {!refundFailed}, Transfer completed: {!transferFailed}. Last error: {ex.Message}");

                _logger.LogError(ex,
                    $"Partial refund FAILED for Payment ID: {payment.Id} after max attempts. " +
                    $"Refund completed: {!refundFailed}, Transfer completed: {!transferFailed}. Status changed to Failed.");
            }
            else
            {
                payment.ScheduledRefundDate = DateTime.UtcNow.AddHours(1);
                payment.Notes = AddNote(payment.Notes,
                    $"Partial refund attempt failed. Refund: {(refundFailed ? "FAILED" : "OK")}, Transfer: {(transferFailed ? "FAILED" : "OK")}. Error: {ex.Message}. Retry scheduled.");

                _logger.LogWarning(ex,
                    $"Partial refund attempt failed for Payment ID: {payment.Id}. " +
                    $"Refund: {(refundFailed ? "FAILED" : "OK")}, Transfer: {(transferFailed ? "FAILED" : "OK")}. " +
                    $"Retry scheduled for {payment.ScheduledRefundDate:yyyy-MM-dd HH:mm:ss}. Error: {ex.Message}");
            }
